...
class biNorm:
  ...
  def __init__(self, df, i, initial=fitted_params):
    ...
    # For plotting
    self.xx = np.linspace(np.min(self.division),
                          np.max(self.division), 1000)
  ...
  def plot_hist(self):
    ...
    '''
    plt.hist(self.division_[:-1], self.division_,
             weights=self.count, label='Data')
...
for df, i in zip(dataframes, range(1, 1 + len(dataframes))):
  ...
  derivative(df).plot(label="Derivative",
                      legend=True, ax=axes[1], style="g-")
